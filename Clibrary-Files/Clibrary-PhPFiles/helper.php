...
function AddNewGroup($AdminID,$AdminName,$usersIDs,$Name)
{
	...
	if ($conn->query($mysql_qry) === TRUE) {
		$mysql_qry1 = "SELECT * FROM usersnotifications where userID in (".$usersIDs.") and userID <>".$AdminID ;
		$result = mysqli_query($conn ,$mysql_qry1);
		if($result!=null){
			if(mysqli_num_rows($result) > 0) {
				while($row = mysqli_fetch_assoc($result)) {
...
function AcceptFriendRequest($ID,$FriendID,$Name)
{
	...
	$mysql_qry1 ="UPDATE user_requestes set Accepted=1 where UserID='$FriendID' and RequestedUserID='$ID' and Accepted=0; 
						 ...
						 INSERT INTO friends (UserID,FriendID,Accepted) VALUES ('$FriendID', '$ID', '1');";
...
{
require "conn.php";
	$mysql_qry = "SELECT u.*,CASE WHEN u.ID IN (select RequestedUserID from user_requestes where userID=".$ID.") THEN 1 ELSE 0 END AS Accepted FROM users u WHERE u.ID <>".$ID." and u.id not in (select friends.FriendID from friends where friends.UserID=".$ID." or friends.FriendID=".$ID." ) order by u.Name asc";
	$result = mysqli_query($conn ,$mysql_qry);
	$arr = array('user' => array());
	if(mysqli_num_rows($result) > 0) {
		while($row = mysqli_fetch_assoc($result)) {
...
define('API_ACCESS_KEY', 'AIzaSyCss3TZymoqcXBnmewEXbtpU5hD2svDxXk');
