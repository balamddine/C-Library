...
function GetUserCategories($userID)
{
	...
	if(mysqli_num_rows($result) > 0) {
		...
		echo json_encode($arr);
			...
			$arr = array("categories" => array(array("status" => "0","message"=>"No categories available","call" => "GetFriendsRequest")));
			echo json_encode($arr);
	}
	$conn->close();
}
...
function AcceptFriendRequest($ID,$FriendID,$Name)
{
	...
	if(mysqli_num_rows($result) > 0) {
		while($row = mysqli_fetch_assoc($result)) {
			...
			$mysql_qry1 ="UPDATE friends set Accepted=1 where UserID='$FriendID' and FriendID='$ID' and Accepted=0; 
...
{
	require "conn.php";
		$mysql_qry ="DELETE FROM friends  where UserID=$ID and FriendID=$FriendID and Accepted=0 ";
		if ($conn->query($mysql_qry) === TRUE) {
...
{
	require "conn.php";
	$mysql_qry = "SELECT users.*,friends.Accepted FROM users inner join friends on friends.UserID=users.ID WHERE friends.FriendID=".$userID." and friends.Accepted=0";
	$result = mysqli_query($conn ,$mysql_qry);
...
function AddAsAFriend($ID,$FriendID,$Name)
{
	...
	if(mysqli_num_rows($result) > 0) {
		while($row = mysqli_fetch_assoc($result)) {
			...
			$mysql_qry1 ="INSERT INTO friends (UserID,FriendID,Accepted) VALUES ('$ID', '$FriendID', '0');";
	if ($conn->query($mysql_qry1) === TRUE) {
...
{
require "conn.php";
	$mysql_qry = "SELECT u.* FROM users u WHERE u.ID !=".$ID." and u.id not in (select friends.FriendID from friends where friends.UserID=".$ID." or friends.FriendID=".$ID." and friends.Accepted=1 ) order by u.Name asc";
	$result = mysqli_query($conn ,$mysql_qry);
	$arr = array('user' => array());
	if(mysqli_num_rows($result) > 0) {
		while($row = mysqli_fetch_assoc($result)) {
...
function register($name,$email,$password,$image,$profession,$notificationtocken,$FBuserid,$call)
{
	...
	if (strpos($image, 'http') !== false) {
		...
		{
			$ch = curl_init($image);
			$fp = fopen(__DIR__ .'/Library/images/'.$fname, 'wb');
...
function GetUser($name,$email,$password,$image,$profession,$notificationtocken,$FBuserid)
{
	...
	if(mysqli_num_rows($result) > 0) {
		...
		$mysql_qry1 = "delete from usersnotifications where userID = '".$row["ID"]."';
						  INSERT INTO usersnotifications(userID,token) VALUES ('".$row["ID"]."', '$notificationtocken');";
			if (mysqli_multi_query($conn,$mysql_qry1)){
				$arr = array("user" => array(array("ID"=>$row["ID"],"Name"=>$row["Name"],"Email"=>$row["Email"],"Password"=>$row["Password"],"Image"=>$row["Image"],"Profession"=>$row["Profession"],"status" => "1","call" => "GetUser")));
				echo json_encode($arr);
			}
		}
	}
